  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      ...
      <main className="container mx-auto py-8 px-4">
        ...
        <Card className="mb-8">
          <CardHeader>
            <CardTitle className="text-2xl">{doctor.first_name} {doctor.last_name}</CardTitle>
          </CardHeader>
          <CardContent>
            <p className="mb-2">Email: {doctor.email}</p>
            <p className="mb-2">Phone: {doctor.phone_number}</p>
            <p className="mb-2">License Number: {doctor.profile.license_number}</p>
            <p className="mb-2">Status: {doctor.profile.is_active ? 'Active' : 'Inactive'}</p>
            <p className="text-sm text-muted-foreground">
              Specialities: {doctor.specialities.map((s: { name: any; }) => s.name).join(', ')}
            </p>
          </CardContent>
        </Card>
